...
namespace UnityEditor.Tilemaps
{
    internal static class TilePaletteWhiteboxSamplesUtility
    {
        ...
        private static List<String> m_WhiteboxSamplesPackages = new List<String>(new[] {"com.unity.2d.tilemap", "com.unity.2d.tilemap.extras"});
        ...
        private static void GetWhiteboxSamples()
        {
            ...
            var search = PackageManager.Client.List( true, false);
            while (!search.IsCompleted)
            {
                Thread.Sleep(100);
            }

            foreach (var result in search.Result)
            {
                if (!m_WhiteboxSamplesPackages.Contains(result.name))
                    continue;
                var samples = Sample.FindByPackage(result.name, result.version);
